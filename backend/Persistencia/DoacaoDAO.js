...
export default class DoacaoDAO{
    ...
    async gravar(doacao){
        if(doacao instanceof Doacao){
            ...
            const sql = `INSERT INTO Doacao (doac_tipo,doac_end,doac_numend,doac_cep,doac_quantidade,doac_data,doac_desc) VALUES ("${doacao.tipo}","${doacao.end}",${doacao.numend},${doacao.cep},${doacao.quant},"${doacao.data.toString()}","${doacao.desc}")`
            console.log(sql);
    ...
    async atualizar(doacao){
        if(doacao instanceof Doacao){
            const dao = new Dao();
            const sql = "UPDATE Doacao SET doac_tipo='"+doacao.tipo+"',doac_end='"+doacao.end+"',doac_numend="+doacao.numend+",doac_cep="+doacao.cep+",doac_quantidade="+doacao.quant+",doac_data='"+doacao.data+"',doac_desc='"+doacao.desc+"' WHERE doac_codigo='"+doacao.codigo;
